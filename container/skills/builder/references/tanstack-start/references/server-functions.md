...
const globalMiddleware = createMiddleware({ type: 'function' }).server(async ({ next }) => {
...
- **Client code**: `import.meta.env.VITE_VAR_NAME` (VITE\_ prefix only)
...
function UserList() {
  ...
  return <button onClick={() => createUser('New User')}>Add User</button>;
}
```
