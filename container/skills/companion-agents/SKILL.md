---
name: companion-agents
description: Spawn long-lived companion or collaborator agents via `create_agent`. Use when the user asks for a persistent helper (researcher, calendar agent, reviewer) or when work should run in parallel as its own agent with its own memory. Distinct from one-off Task sub-agents — these persist across sessions.
---
